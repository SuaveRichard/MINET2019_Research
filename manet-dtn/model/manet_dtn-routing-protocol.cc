...
/*  file = fopen("data/RecvCount.csv","a");
 ...
 RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
 {
  ...
  /*  if(5 < m_ipv4->GetObject<Node>()->GetId() && m_ipv4->GetObject<Node>()->GetId()< 12)
   {
     m_protcol = 0;
   }
   else
   ...
   iter != listB.m_packets.end(); iter++)
   {
     ...
     if(newEntry.GetPacket())
     {
      ...
      if (m_ipv4->GetInterfaceForAddress (m_mainAddress) != -1)
        ...
        dataCount_send++;
        std::cout<<"ProcessSV: sender " << senderIface << " receiver " << m_mainAddress<<std::endl;
        NS_LOG_FUNCTION("ProcessSV: sender " << senderIface << " receiver " << m_mainAddress);
        ucb(rt,pkt,header);
  ...
  if(m_protcol == 0)
  {
    ...
    if(tag.GetTagType() != ControlTag::CONTROL)
    {
      ...
      if(m_queue.Find(packet->GetUid()).GetPacketID() == 0)
      {
        ...
        NS_LOG_FUNCTION("Delay: "<<Simulator::Now() - Seconds(20) << " DTNNode " << NodeNumber << " Position: " << 
        m_ipv4->GetObject<Node>()->GetObject<MobilityModel>()->GetPosition().x);
      }
    }
