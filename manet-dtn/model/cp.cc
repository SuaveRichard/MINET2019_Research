/* -*- Mode:C++; c-file-style:"gnu"; indent-tabs-mode:nil; -*- */
 ...
 RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
 {
  ...
  /*  if(m_ipv4->GetObject<Node>()->GetId() == 3 ||
       m_ipv4->GetObject<Node>()->GetId() == 4 ||
       m_ipv4->GetObject<Node>()->GetId() == 11 || 
       m_ipv4->GetObject<Node>()->GetId() == 12 || 
       m_ipv4->GetObject<Node>()->GetId() == 19 || 
       m_ipv4->GetObject<Node>()->GetId() == 20)
   {
     m_protcol = 0;
   }*/
   m_protcol = 1;
  ...
  if(listA.Size() != 0)
  {
   ...
   iter != listB.m_packets.end(); iter++)
   {
     ...
     if(newEntry.GetPacket())
     {
      ...
      ControlTag tempTag(ControlTag::DATA);
                             ...
                             const Ipv4Address &senderIface)
 {
  ...
  /* for (std::vector<Ipv4Address>::const_iterator i = tc.neighborAddresses.begin ();
     ...
     {*/
     //std::cout<<"tc.neighborAddresses.size() = " << tc.neighborAddresses.size() << std::endl;
    // std::cout<<"tc.neighborPositon.size() = " << tc.neighborPositions.size() << std::endl;
    // std::cout<<"tc.neighborVTypes.size() = " << tc.neighborVTypes.size() << std::endl;
 ...
 void
 RoutingProtocol::MprBroadcast(Ptr<Packet> packet)
 {
   NS_LOG_FUNCTION(this);
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
          m_sendSockets.begin (); i != m_sendSockets.end (); i++)
 ...
 RoutingProtocol::SendTc ()
 {
  NS_LOG_FUNCTION (this);
   manet_dtn::MessageHeader msg;
   //std::cout<<"Node"<<m_ipv4->GetObject<Node>()->GetId() <<"がMPRです" << std::endl;
   msg.SetVTime (manet_dtn_TOP_HOLD_TIME);
   msg.SetOriginatorAddress (m_mainAddress);
 ...
 uint16_t nextAC_position = 1000;
 uint16_t lastposition = 1000;
 bool findAC = false;
 Vector position = m_ipv4->GetObject<Node> ()->GetObject<MobilityModel>()->GetPosition();
 /*自ノードがMPRであるならばデータをブロードキャスト*/
 /*if (m_state.GetMprSelectors ().size () > 0)
 {
 ...
 for(std::map<Ipv4Address, RoutingTableEntry>::const_iterator it = m_table.begin();
     it != m_table.end(); it++)
     {
      ...
      if(position.x > it->second.position)
       {
         if(it->second.vType == 1&& nextAC_position > it->second.position)
         {
          nextAC = it->first;
          nextAC_position = it->second.position;
          findAC = true;
         }
       }
     }
   ...
   if(tag.GetTagType() == ControlTag::CONTROL)
  {
   ...
   if (numOifAddresses == 1)
    ...
    NS_LOG_DEBUG(rtentry->GetDestination()<< " " << rtentry->GetGateway());
    return rtentry;
  }
  if (Lookup (header.GetDestination (), entry1) != 0)
     {
       if(findAC) Lookup(nextAC,entry1);
    ...
    {
       if(findAC) Lookup(nextAC,entry1);
       else Lookup(lastposition_vehicle,entry1);
       bool foundSendEntry = FindSendEntry (entry1, entry2);
       if (!foundSendEntry)
         {
       ...
       rtentry = Create<Ipv4Route> ();
       rtentry->SetDestination (lastposition_vehicle);
       NS_ASSERT (m_ipv4);
       uint32_t numOifAddresses = m_ipv4->GetNAddresses (interfaceIdx);
       NS_ASSERT (numOifAddresses > 0);
       Ipv4InterfaceAddress ifAddr;
     ...
     if(tag.GetTagType() == ControlTag::NOT_SET)
      {
       ...
       newEntry.SetExpireTime(Time (Seconds (50000) + Simulator::Now ()));
       if(m_queue.Find(packet->GetUid()).GetPacketID() == 0)
        {
         std::cout<<Simulator::Now() - Seconds (20) << " にNode"<<m_ipv4->GetObject<Node>()->GetId() << "に届きました" << std::endl;
         m_queue.Enqueue(newEntry);
        }
      }
      /*this is a Contorol(Hello or TC or SV or DREQ) Message*/
      else
      {
...
/*DATAならQueueへ追加する処理、Controlなら上位層へ*/
      ...
      if (!lcb.IsNull ())
          {
            if(tag.GetTagType() == ControlTag::NOT_SET)
            {
              if(header.GetDestination() == "10.255.255.255")
              {
                std::cout<<"Mprからの受信\n";
              } 
              else
            ...
            if(m_queue.Find(packet->GetUid()).GetPacketID() == 0)
              {
                ...
                if (m_state.GetMprSelectors ().size () > 0)
                {
                std::thread th(&RoutingProtocol::MprBroadcast,this,packet);
                th.detach();
                }
              }
            }
   ...
   if(Lookup(header.GetDestination(),entry1) == 0)
     {
       /*宛先が見つからず後方車両が存在しない場合、自身を宛先としてlocal delivery*/
      if(!forward)
        {
        ...
        dst = iface.GetLocal();/*自ノードアドレスを取得*/
        iif = m_ipv4->GetInterfaceForDevice (idev);
        NS_LOG_DEBUG("No found dest , so lcb for" << dst);
        QueueEntry newEntry (packet, header, ucb, ecb);
        newEntry.SetPacketID(packet->GetUid());
        newEntry.SetExpireTime(Time (Seconds (50000) + Simulator::Now ()));
        m_queue.Enqueue(newEntry);
        ...
        /*宛先が見つからず後方車両が存在する場合、最後方車両を宛先としてunicast*/
        else
        {
          dst = lastposition_vehicle;
          if(findAC)  Lookup(nextAC,entry1);
          else Lookup(lastposition_vehicle,entry1);
            bool foundSendEntry = FindSendEntry (entry1, entry2);
            if (!foundSendEntry)
           ...
           NS_LOG_DEBUG(rtentry->GetGateway() <<"に転送しました1");
           QueueEntry newEntry (packet, header, ucb, ecb);
        newEntry.SetPacketID(packet->GetUid());
        newEntry.SetExpireTime(Time (Seconds (50000) + Simulator::Now ()));
        m_queue.Enqueue(newEntry);
