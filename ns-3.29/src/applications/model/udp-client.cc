...
UdpClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpClient")
    ...
    .AddAttribute ("NodeNumber",
                   "The Node number of pakets send",
                   UintegerValue (0),
                   MakeUintegerAccessor (&UdpClient::m_Nodenum),
                   MakeUintegerChecker<uint32_t> ())
    ;
  return tid;
}
...
*/
  ...
  if ((m_socket->Send (p)) >= 0)
    {
      ++m_sent;
      NS_LOG_INFO ("TraceDelay: TX NodeId: "<< m_Nodenum << " " 
                                    << m_size << " bytes to "  
                                    << Ipv4Address::ConvertFrom (m_peerAddress)
                                    << " Uid: " << p->GetUid () << " Time: "
                                    << (Simulator::Now ()).GetSeconds ());

      std::cout << "TX" << t_count << ", NodeId, "<< m_Nodenum << " ,packet size, " 
                                    << m_size << ", bytes to, "  
                                    << Ipv4Address::ConvertFrom (m_peerAddress)
                                    << ", Uid, " << p->GetUid () << ", Time, "
                                    << (Simulator::Now ()).GetSeconds ()
    ...
    {
      NS_LOG_INFO ( "Error while sending NodeId: " << m_Nodenum << " "
                                          << m_size << " bytes to "
