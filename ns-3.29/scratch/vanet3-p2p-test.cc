...
void
ReceiveFromDevice (bool promisc, Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol,
...
void
PromiscReceive (Ptr<NetDevice> wifiNetDevice, bool promisc,
                ProtocolHandler handler, uint16_t protocolType)
{
  wifiNetDevice->SetPromiscReceiveCallback(MakeCallback(&ReceiveFromDevice,this));
}
*/


double GetRandRange(uint64_t min_val, uint64_t max_val ) {
    // 乱数生成器
    ...
    // 乱数を生成
    uint64_t rand = get_rand_uni_int(mt64);
    double return_rand = double(rand);
...
/*
  ...
  Ptr<WifiNetDevice> wifinetdevice = dev->GetObject<WifiNetDevice> ();
  wifinetdevice->SetReceiveCallback (MakeCallback (&ReceivePacket));
  wifinetdevice->SetPromiscReceiveCallback(MakeCallback(&ReceivePacket));
*/


  //p2p設定
  PointToPointHelper p2pdown;
  p2pdown.SetDeviceAttribute ("DataRate",StringValue ("28.29Mbps"));
  p2pdown.SetChannelAttribute ("Delay", StringValue ("25.1ms"));

  PointToPointHelper p2pup;
  p2pup.SetDeviceAttribute ("DataRate",StringValue ("2.33Mbps"));
  p2pup.SetChannelAttribute ("Delay", StringValue ("25.1ms"));

  NetDeviceContainer p2pdevices, p2pdevices2, p2pdevices3, p2pdevices4;

  if(scenario == 1){

    p2pdevices = p2pup.Install (vehicleNodes1.Get (0), p2pNodes.Get (0));
    p2pdevices2 = p2pdown.Install (p2pNodes.Get (0), vehicleNodes2.Get (0));
    p2pdevices3 = p2pup.Install (vehicleNodes1.Get (0), p2pNodes.Get (1));
    p2pdevices4 = p2pdown.Install (p2pNodes.Get (1), vehicleNodes3.Get (0));

  }else if(scenario == 2){

    p2pdevices = p2pup.Install (vehicleNodes1.Get (numvehicleNodes1 - 1), p2pNodes.Get (0));
    p2pdevices2 = p2pdown.Install (p2pNodes.Get (0), vehicleNodes2.Get (0));
    p2pdevices3 = p2pup.Install (vehicleNodes2.Get (numvehicleNodes2 - 1), p2pNodes.Get (1));
    p2pdevices4 = p2pdown.Install (p2pNodes.Get (1), vehicleNodes3.Get (0));
...
//ランダム配置
  ...
  for(uint32_t j = 0; j < (numvehicleNodes1/2); j++){
    positionAlloc1->Add (Vector (v1_x, 0.0, 0.0));
    v1_x = v1_x + GetRandRange(20,40);
    positionAlloc1->Add (Vector (v1_x, 5.0, 0.0));
    v1_x = v1_x + GetRandRange(20,40);
  }

  vehiclemobility1.SetPositionAllocator (positionAlloc1);
  vehiclemobility1.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  vehiclemobility1.Install (vehicleNodes1);
  ...
  for(uint32_t k = 0; k < (numvehicleNodes2/2); k++){
    positionAlloc2->Add (Vector (v2_x, 0.0, 0.0));
    v2_x = v2_x + GetRandRange(20,40);
    positionAlloc2->Add (Vector (v2_x, 5.0, 0.0));
    v2_x = v2_x + GetRandRange(20,50);
  }

  vehiclemobility2.SetPositionAllocator (positionAlloc2);
  vehiclemobility2.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  vehiclemobility2.Install (vehicleNodes2);
  ...
  for(uint32_t l = 0; l < (numvehicleNodes3/2); l++){
    positionAlloc3->Add (Vector (v3_x, 0.0, 0.0));
    v3_x = v3_x + GetRandRange(20,40);
    positionAlloc3->Add (Vector (v3_x, 5.0, 0.0));
    v3_x = v3_x + GetRandRange(20,40);
  }

  vehiclemobility3.SetPositionAllocator (positionAlloc3);
  vehiclemobility3.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  vehiclemobility3.Install (vehicleNodes3);
...
//p2p application
  ...
  if(scenario == 1){
    ...
    clientApps1.Start (Seconds (30.0));
...
//create vanet packets 
  ...
  sourceApps1.Start (Seconds (30.0));
  ...
  sourceApps2.Start (Seconds (30.0));
  ...
  sourceApps3.Start (Seconds (30.0));
...
/*
    NS_LOS_INFO( "server node address " << p2pInterface.GetAddress (1) 
              << " " << p2pInterface2.GetAddress (0) << "\n";
    NS_LOS_INFO( "vehicle2/p2p node address " << p2pInterface2.GetAddress (1) << "\n";
...
/*  
  	for(uint32_t i = 0; i < numvehicleNodes1; i++){
	  	...
	  	NS_LOS_INFO( "vehicle1 nodes node " << i << " address " 
	  			  << olsrInterface1.GetAddress (i) << "\n";
	}
    for(uint32_t i = 0; i < numvehicleNodes2; i++){

      NS_LOS_INFO( "vehicle2 nodes node " << (i + numvehicleNodes1) << " address " 
            << olsrInterface2.GetAddress (i) << "\n";
  }

    for(uint32_t i = 0; i < numvehicleNodes3; i++){

      NS_LOS_INFO( "vehicle3 nodes node " << (i + numvehicleNodes2) << " address " 
            << olsrInterface3.GetAddress (i) << "\n";
  }


	NS_LOS_INFO( "client1 node address " << p2pInterface1.GetAddress (0));
  NS_LOS_INFO( "server1 node address " << p2pInterface1.GetAddress (1));
  NS_LOS_INFO( "client2 node address " << p2pInterface2.GetAddress (0));
  NS_LOS_INFO( "server2 node address " << p2pInterface2.GetAddress (1));
  NS_LOS_INFO( "create3 node address " << p2pInterface3.GetAddress (0));
  NS_LOS_INFO( "server3 node address " << p2pInterface3.GetAddress (1));
  NS_LOS_INFO( "client4 node address " << p2pInterface4.GetAddress (0));
  NS_LOS_INFO( "server4 node address " << p2pInterface4.GetAddress (1));

  NS_LOS_INFO( "serveraddress1 " << Ipv4Address::ConvertFrom (serverAddress1) << );
  NS_LOS_INFO( "serveraddress2 " << Ipv4Address::ConvertFrom (serverAddress2) << );
  NS_LOS_INFO( "serveraddress3 " << Ipv4Address::ConvertFrom (serverAddress3) << );
  NS_LOS_INFO( "serveraddress4 " << Ipv4Address::ConvertFrom (serverAddress4) << );
	...
	for (NodeContainer::Iterator j = vehicleNodes1.Begin (); j != vehicleNodes1.End (); ++j)
    {
       ...
       NS_LOS_INFO( "vehicle node" << num1 << " x=" << pos.x << ", y=" << pos.y << );
       num1++;
     }
