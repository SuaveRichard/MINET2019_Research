...
int main (int argc, char *argv[])
{
  ...
  p2pNodes.Create (1);
/*
  p2pNodes.Get (0) = olsrNodes.Get (0);
...
*/
  ...
  p2pdevices = p2p.Install (olsrNodes.Get (0), csmaNodes.Get (0));
  p2pdevices1 = p2p.Install (p2pNodes.Get (0), csmaNodes.Get (0));
  ...
  positionAlloc1->Add (Vector (5.0, 10.0, 0.0));
  mobility1.SetPositionAllocator (positionAlloc1);
  mobility1.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobility1.Install (csmaNodes);

  MobilityHelper mobility2;
  Ptr<ListPositionAllocator> positionAlloc2 = CreateObject<ListPositionAllocator> ();
  positionAlloc2->Add (Vector (10.0, 15.0, 0.0));
  mobility2.SetPositionAllocator (positionAlloc2);
  mobility2.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobility2.Install (p2pNodes);


//  Ns2MobilityHelper ns2 = Ns2MobilityHelper ("/home/fk/sumo/tools/test1mobility.tcl");
//  ns2.Install ();
  ...
  olsrInterface = ipv4.Assign (devices);


  Ipv4AddressHelper serverIpv4;
  serverIpv4.SetBase ("11.1.1.0", "255.255.255.0");

  Ipv4InterfaceContainer serverinterface;
  serverinterface = serverIpv4.Assign (csmaDevices);
  ...
  address1.SetBase ("13.1.1.0", "255.255.255.0");

  Ipv4InterfaceContainer p2pinterface1 = address1.Assign (p2pdevices1);
...
//Ipv4InterfaceContainer interface = address.Assign (devices);
  ...
  UdpServerHelper server (1000);
  ApplicationContainer serverApps = server.Install(csmaNodes.Get (0));
  serverApps.Start (Seconds (1.0));
  serverApps.Stop (Seconds (60.0));

  UdpServerHelper server1 (1111);
  ApplicationContainer serverApps1 = server1.Install(csmaNodes.Get (0));
  serverApps1.Start (Seconds (1.0));
  serverApps1.Stop (Seconds (60.0));

  UdpClientHelper client1 (p2pinterface.GetAddress (1),1000);
  client1.SetAttribute ("MaxPackets", UintegerValue (1));
  client1.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
  client1.SetAttribute ("PacketSize", UintegerValue (1024));

  ApplicationContainer clientApps = client1.Install (olsrNodes.Get(0));
  clientApps.Start (Seconds (1.5));
  clientApps.Stop (Seconds (60.0));

  UdpClientHelper client2 (p2pinterface1.GetAddress (1),1111);
...
for(uint32_t i = 0; i < 2; i++){
  std::cout << "olsr node " << i << " address " << olsrInterface.GetAddress (i) << "\n";
}

std::cout << "server node address " << serverinterface.GetAddress (0) << "\n";
std::cout << "server node address2 " << p2pinterface.GetAddress (0) << "\n";
std::cout << "p2p node address " << p2pinterface1.GetAddress (0) << "\n";
  ...
  AnimationInterface anim ("my-olsr.xml");
  Simulator::Run ();
  Simulator::Destroy ();
